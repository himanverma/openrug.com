                ...
                if (is_array($libs)) {
                    foreach ($libs as $lib) {
                        if (strpos($lib, '//') === false) {
                            ...
                            $lib = $this->assetUrl($lib, array('pathPrefix' => Configure::read('App.'.$type.'BaseUrl'), 'ext' => '.'.$type));
                            //$lib =  WWW_ROOT . ltrim($lib,"/"); //---Get From Local
                            $lib = $this->url("/",true) . ltrim($lib,"/");  // --- Get From domain 
                        }
                        if ($toEnd) {
